...
st.sidebar.title("MK_RSI_OBV")
st.sidebar.caption(f"🌐 실전투자 서버: `{BASE_URL}`")
st.sidebar.divider()

st.sidebar.subheader("🎯 조건선택")
...
def get_latest_trading_day():
    """주말/새벽이면 가장 가까운 직전 거래일 반환"""
    now = datetime.now()
    # 새벽 (장 마감 전: 09:00 이전이면 전날 데이터 사용)
    if now.hour < 9:
...
target_date    = st.sidebar.date_input("조회 날짜", default_date)
formatted_date = target_date.strftime("%Y%m%d")

# 날짜 자동 조정 안내
if default_date.date() != datetime.today().date():
    st.sidebar.info(f"📅 자동 조정: {default_date.strftime('%Y/%m/%d')} (직전 거래일)")
...
def get_sector_and_marcap(code: str, token: str = "", base_dt: str = ""):
    ...
    if marcap_str == "-":
        try:
            ...
            res = requests.get(url, headers=NAVER_HEADERS, timeout=8)
            soup = BeautifulSoup(res.text, "html.parser")
            tag = soup.select_one("#_market_sum")
            if tag:
                text = tag.get_text(strip=True).replace(" ", "")
                jo, eok = 0, 0
                if "조" in text:
                    parts = text.split("조", 1)
                    jo = int(parts[0].replace(",", "")) if parts[0].replace(",","").isdigit() else 0
                    if len(parts) > 1 and parts[1].replace(",","").replace("억","").isdigit():
                        eok = int(parts[1].replace(",","").replace("억",""))
                else:
                    num = text.replace(",","").replace("억","")
                    if num.isdigit():
                        eok = int(num)
                total_won = (jo * 10000 + eok) * 100_000_000
                if total_won > 0:
                    marcap_str, marcap_num = _fmt_marcap(total_won)
        except Exception:
            pass
    ...
    with st.status("분석 중...", expanded=True) as status:
        ...
        for i, s in enumerate(pre_filtered):
            ...
            try:
                ...
                # ✅ 매수신호 조건 강화
                # RSI 크로스 3봉 이하 AND OBV 크로스 3봉 이하 AND 거래대금 500억 이상
                is_strong_buy = (
                    is_sig and
                    rsi_ago <= 3 and
                    obv_ago <= 3 and
                ...
                final_results.append({
                    "신호":          "강력매수" if is_strong_buy else ("관심" if is_sig else "⬜"),
                    "섹터":          sector,
                    "종목명":        s.get("stk_nm") or "-",
                    "코드":          code,
                    "현재가":        prc_display,
        ...
        if final_results:
            ...
            def bg_color(row):
                is_strong_buy = row.get("신호") == "강력매수"
                is_watch      = row.get("신호") == "관심"
                is_big_money  = money_num.iloc[row.name] >= 500

                if is_strong_buy:
                    return ["background-color:#FFD6E0"] * len(row)  # 분홍
                elif is_watch and is_big_money:
                    return ["background-color:#FFF3CD"] * len(row)  # 노랑
                return [""] * len(row)
            ...
            legend_df = pd.DataFrame({
                "신호": ["강력매수", "관심", "🟡 노랑"],
                "의미": [
                    "거래대금 500억↑ & RSI & OBV 3봉 이하 골든크로스 유지",
                    "RSI & OBV 골든크로스 유지",
                    "거래대금 500억↑ 주도주",
                ]
            })

            def legend_color(row):
                if row["신호"] == "강력매수":
            ...
            st.dataframe(
                legend_df.style.apply(legend_color, axis=1),
            ...
            # 자동 저장 (실행 폴더에 바로 저장)
            save_path = f"scan_{formatted_date}.csv"
            df_display.to_csv(save_path, index=False, encoding="utf-8-sig")
            st.success(f"💾 CSV 자동 저장 완료: `{save_path}`")

            # 다운로드 버튼도 유지
            st.download_button(
                label=f"📥 CSV 다운로드 (scan_{formatted_date}.csv)",
                data=csv_buffer,
                file_name=f"scan_{formatted_date}.csv",
                mime="text/csv",
            )
